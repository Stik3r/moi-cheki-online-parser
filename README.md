...
### Параметры `getReceipts`

Обязательные поля: `limit` — количество чеков, `offset` — смещение для пагинации, `orderBy` — порядок сортировки.

Допустимые значения `orderBy`: `CREATED_DATE:ASC`, `CREATED_DATE:DESC`, `TOTAL_SUMM:ASC`, `TOTAL_SUMM:DESC`.
